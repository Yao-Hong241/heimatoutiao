      ...
      <div class="img_list">
        <div class="img_item" v-for="item in images" :key="item.id">
          ...
          <div class="option" v-if="!filterParams.collect">
            <span @click="toggleStatus(item)" class="el-icon-star-on" :class="{blue:item. is_collected}"></span>
    ...
    <el-dialog title="上传素材" :visible.sync="dialogVisible" width="300px">
      <span>
        <el-upload
          ...
          name="image">
          <!-- img 预览图  i 是上传按钮  imageUrl数据 有值的时候预览图片 没有值是上传按钮 -->
...
export default {
  ...
  methods: {
    ...
    // 获取图片数据
    async getImages () {
      const { data: { data } } = await this.$http.get('user/images', { params: this.filterParams })
